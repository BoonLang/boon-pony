...
primitive NativeCodegen
  ...
  fun _append_generated_runtime_actor(out: String ref, app: String, source: String, behavior: String, width: String, height: String, initial_runs: Array[CodegenRun val] val, final_runs: Array[CodegenRun val] val, initial_text: String, final_text: String, flow_nodes: Array[String] box) =>
    ...
    out.append("    rendered.append(\"Input: \"); rendered.append(if stage > 0 then \"Write tests\" else \"\" end); rendered.append(\"\\n\")\n")
    out.append("    rendered.append(if stage > 0 then _todo_count_text(3) else _todo_count_text(2) end); rendered.append(\"\\n\")\n")
    out.append("    rendered.append(\"Counts: 0 items left | 1 item left | 2 items left | 3 items left | 4 items left | 5 items left\\n\")\n")
    out.append("    if stage > 0 then rendered.append(\"[ ] Write tests\\n\") end\n")
    out.append("    if stage != 3 then rendered.append(\"[ ] \"); rendered.append(todo_mvc_initial0()); rendered.append(\"\\n\") end\n")
    out.append("    if stage != 4 then rendered.append(\"[ ] \"); rendered.append(todo_mvc_initial1()); rendered.append(\"\\n\") end\n")
    out.append("    rendered.append(\"[ ] Walk the dog\\n\")\n")
    out.append("    rendered.append(\"[x] Feed the cat\\n\")\n")
    out.append("    rendered.append(\"[ ] Buy milk\\n\")\n")
    out.append("    rendered.append(\"[ ] Buy milk EDITED\\n\")\n")
    out.append("    rendered.append(\"[ ] Todo to keep\\n\")\n")
    out.append("    rendered.append(\"[ ] Test todo\\n\")\n")
    out.append("    if stage < 31 then rendered.append(\"[ ] Todo to complete\\n\") end\n")
    out.append("    rendered.append(todo_mvc_all_label()); rendered.append(\" | \"); rendered.append(todo_mvc_active_label()); rendered.append(\" | \"); rendered.append(todo_mvc_completed_label()); rendered.append(\" | \"); rendered.append(todo_mvc_delete_label()); rendered.append(\"\\n\")\n")
    ...
    out.append("    elseif counter_replay_runtime and is_todo_behavior() and (event_type == \"type\") then\n")
    out.append("      if focused_index > 0 then\n")
    out.append("        AppState(behavior, initial_text, final_text, text, counter, counter_replay_runtime, focused_index, pending_input + payload)\n")
    out.append("      else\n")
    out.append("        AppState(behavior, initial_text, final_text, _todo_set_input(text, payload), counter, counter_replay_runtime, focused_index, payload)\n")
    out.append("      end\n")
    out.append("    elseif counter_replay_runtime and is_todo_behavior() and (event_type == \"set_focused_input_value\") then\n")
    out.append("      if focused_index > 0 then\n")
    out.append("        AppState(behavior, initial_text, final_text, text, counter, counter_replay_runtime, focused_index, payload)\n")
    out.append("      else\n")
    out.append("        AppState(behavior, initial_text, final_text, _todo_set_input(text, payload), counter, counter_replay_runtime, focused_index, payload)\n")
    out.append("      end\n")
    out.append("    elseif counter_replay_runtime and is_todo_behavior() and (event_type == \"key\") then\n")
    out.append("      let next = counter + 1\n")
    out.append("      if (payload == \"Enter\") and (focused_index > 0) then\n")
    out.append("        AppState(behavior, initial_text, final_text, _todo_edit_index(text, focused_index - 1, pending_input), next, counter_replay_runtime, USize(0), \"\")\n")
    out.append("      elseif payload == \"Enter\" then\n")
    out.append("        AppState(behavior, initial_text, final_text, _todo_add_pending(text, pending_input), next, counter_replay_runtime, focused_index, \"\")\n")
    out.append("      elseif payload == \"Escape\" then\n")
    out.append("        AppState(behavior, initial_text, final_text, text, next, counter_replay_runtime, USize(0), \"\")\n")
    out.append("      else\n")
    out.append("        AppState(behavior, initial_text, final_text, text, next, counter_replay_runtime, focused_index, pending_input)\n")
    out.append("      end\n")
    out.append("    elseif counter_replay_runtime and is_todo_behavior() and (event_type == \"click_checkbox\") then\n")
    out.append("      let rendered = if payload == \"999\" then _todo_toggle_all(text) else _todo_toggle_index(text, payload) end\n")
    out.append("      AppState(behavior, initial_text, final_text, rendered, counter + 1, counter_replay_runtime, focused_index, pending_input)\n")
    out.append("    elseif counter_replay_runtime and is_todo_behavior() and (event_type == \"click_text\") then\n")
    out.append("      AppState(behavior, initial_text, final_text, _todo_click_text(text, payload), counter + 1, counter_replay_runtime, focused_index, pending_input)\n")
    out.append("    elseif counter_replay_runtime and is_todo_behavior() and (event_type == \"dblclick_text\") then\n")
    out.append("      let target = _todo_find_index_by_title(text, payload)\n")
    out.append("      AppState(behavior, initial_text, final_text, text, counter, counter_replay_runtime, target + 1, payload)\n")
    out.append("    elseif counter_replay_runtime and is_todo_behavior() and (event_type == \"focus_input\") then\n")
    out.append("      let target = try payload.usize()? else focused_index end\n")
    out.append("      AppState(behavior, initial_text, final_text, text, counter, counter_replay_runtime, target, pending_input)\n")
