...
primitive NativeCodegen
  ...
  fun _append_generated_runtime_actor(out: String ref, app: String, source: String, behavior: String, width: String, height: String, initial_runs: Array[CodegenRun val] val, final_runs: Array[CodegenRun val] val, initial_text: String, final_text: String, flow_nodes: Array[String] box) =>
    ...
    out.append("    rendered.append(\"Input: \"); if editing_index == 0 then rendered.append(input) end; rendered.append(\"\\n\")\n")
    ...
    out.append("            rendered.append(\"[edit] \"); rendered.append(input); rendered.append(\"\\n\")\n")
    out.append("          else\n")
    out.append("            rendered.append(if completed then \"[x] \" else \"[ ] \" end); rendered.append(_todo_item_title(raw)); rendered.append(\"\\n\")\n")
    out.append("          end\n")
    out.append("        end\n")
    out.append("        index = index + 1\n")
    out.append("      end\n")
    ...
    out.append("    text = if text' == \"\" then if (behavior' == \"numeric_counter\") and initial_text'.contains(\"[ + ]\") then \"0\\n[ + ]\" elseif behavior' == \"spreadsheet_state\" then GeneratedSpreadsheet.text(\"Cells\", counter, aux_counter) elseif behavior' == \"timer_counter\" then \"\" elseif behavior' == \"chained_list_state\" then GeneratedDocumentState.chained_list_text(counter) elseif behavior' == \"crud_state\" then GeneratedDocumentState.crud_text(counter) elseif behavior' == \"todo_mvc_state\" then GeneratedDocumentState.todo_mvc_render(todo_items, todo_filter, pending_input, focused_index) elseif behavior' == \"todo_mvc_physical_state\" then GeneratedDocumentState.todo_mvc_physical_text(counter) else initial_text' end else text' end\n\n")
