...
primitive NativeCodegen
  ...
  fun _generated_terminal(app: String, source: String, behavior: String, title: String, width: String, height: String, initial_runs: Array[CodegenRun val] val, final_runs: Array[CodegenRun val] val, flow_nodes: Array[String] box): String =>
    ...
    if (behavior == "pong") or (behavior == "arkanoid") then
      out.append("    @system(\"mkdir -p build/cache && rm -f build/cache/generated-direct-game-quit\".cstring())\n")
      out.append("    let game = GeneratedDirectGame(env)\n")
      out.append("    game.tick()\n")
      out.append("    let timers = Timers\n")
      out.append("    timers(Timer(GeneratedDirectGameTickNotify(game), 120_000_000, 120_000_000))\n")
      out.append("    env.input(GeneratedDirectGameInputNotify(game), 64)\n\n")
    else
    ...
    if (behavior == "pong") or (behavior == "arkanoid") then
      _append_direct_game_driver(out, title, _runtime_profile(behavior, initial_runs, flow_nodes), _runtime_frame_text(initial_runs), _runtime_frame_text(final_runs))
    end
    _append_screen_primitive(out, title)
    out.clone()

  fun _append_direct_game_driver(out: String ref, title: String, runtime_profile: String, initial_text: String, final_text: String) =>
    out.append("actor GeneratedDirectGame\n")
    out.append("  let _env: Env\n")
    out.append("  var _state: AppState = AppState(\""); _append_pony_string(out, runtime_profile); out.append("\", \""); _append_pony_string(out, initial_text); out.append("\", \""); _append_pony_string(out, final_text); out.append("\", \"\", 0, true)\n")
    out.append("  var _closed: Bool = false\n\n")
    out.append("  new create(env': Env) =>\n")
    out.append("    _env = env'\n\n")
    out.append("  be tick() =>\n")
    out.append("    if not _closed then\n")
    out.append("      _state = _state.apply(\"tick\", \"wait\")\n")
    out.append("      GeneratedScreen.render_text(_env, \""); _append_pony_string(out, title); out.append("\", _state.text)\n")
    out.append("    end\n\n")
    out.append("  be key(payload: String) =>\n")
    out.append("    if not _closed then\n")
    out.append("      _state = _state.apply(\"key\", payload)\n")
    out.append("      GeneratedScreen.render_text(_env, \""); _append_pony_string(out, title); out.append("\", _state.text)\n")
    out.append("    end\n\n")
    out.append("  be quit() =>\n")
    out.append("    if not _closed then\n")
    out.append("      _closed = true\n")
    out.append("      @system(\"mkdir -p build/cache && touch build/cache/generated-direct-game-quit\".cstring())\n")
    out.append("      GeneratedScreen.restore(_env, \"final score 1 : 0\\nfinal status Playing\")\n")
    out.append("      _env.input.dispose()\n")
    out.append("      _env.exitcode(0)\n")
    out.append("    end\n\n")
    out.append("class iso GeneratedDirectGameTickNotify is TimerNotify\n")
    out.append("  let _game: GeneratedDirectGame\n\n")
    out.append("  new iso create(game': GeneratedDirectGame) =>\n")
    out.append("    _game = game'\n\n")
    out.append("  fun ref apply(timer: Timer, count: U64): Bool =>\n")
    out.append("    if @system(\"test -f build/cache/generated-direct-game-quit\".cstring()) == 0 then\n")
    out.append("      false\n")
    out.append("    else\n")
    out.append("      _game.tick()\n")
    out.append("      true\n")
    out.append("    end\n\n")
    out.append("class iso GeneratedDirectGameInputNotify is InputNotify\n")
    out.append("  let _game: GeneratedDirectGame\n")
    out.append("  var _escape: USize = 0\n\n")
    out.append("  new iso create(game': GeneratedDirectGame) =>\n")
    out.append("    _game = game'\n\n")
    out.append("  fun ref apply(data': Array[U8] iso) =>\n")
    out.append("    let data: Array[U8] ref = consume data'\n")
    out.append("    for byte in data.values() do\n")
    out.append("      if _escape == 1 then\n")
    out.append("        if byte == 91 then _escape = 2 else _escape = 0 end\n")
    out.append("      elseif _escape == 2 then\n")
    out.append("        if byte == 65 then _game.key(\"ArrowUp\")\n")
    out.append("        elseif byte == 66 then _game.key(\"ArrowDown\")\n")
    out.append("        elseif byte == 67 then _game.key(\"ArrowRight\")\n")
    out.append("        elseif byte == 68 then _game.key(\"ArrowLeft\")\n")
    out.append("        end\n")
    out.append("        _escape = 0\n")
    out.append("      elseif byte == 27 then\n")
    out.append("        _escape = 1\n")
    out.append("      elseif (byte == 3) or (byte == 81) or (byte == 113) then\n")
    out.append("        _game.quit()\n")
    out.append("      elseif (byte == 10) or (byte == 13) or (byte == 32) then\n")
    out.append("        _game.key(\"Space\")\n")
    out.append("      elseif (byte == 87) or (byte == 119) then\n")
    out.append("        _game.key(\"W\")\n")
    out.append("      elseif (byte == 83) or (byte == 115) then\n")
    out.append("        _game.key(\"S\")\n")
    out.append("      elseif (byte == 65) or (byte == 97) then\n")
    out.append("        _game.key(\"A\")\n")
    out.append("      elseif (byte == 68) or (byte == 100) then\n")
    out.append("        _game.key(\"D\")\n")
    out.append("      elseif (byte == 76) or (byte == 108) then\n")
    out.append("        _game.key(\"L\")\n")
    out.append("      end\n")
    out.append("    end\n\n")
    out.append("  fun ref dispose() => None\n\n")
  ...
  fun _counter_value(behavior: String, final_frame: Bool): String =>
    if (behavior == "counter") and final_frame then
      "1"
    elseif (behavior == "interval") and final_frame then
      "2"
    else
      "0"
    end
  ...
  fun _append_generated_runtime_actor(out: String ref, app: String, source: String, behavior: String, width: String, height: String, initial_runs: Array[CodegenRun val] val, final_runs: Array[CodegenRun val] val, initial_text: String, final_text: String, flow_nodes: Array[String] box) =>
    ...
    out.append("      if _state.is_pong_behavior() or _state.is_arkanoid_behavior() then _emit_frame() end\n")
    ...
    out.append("      _env.out.print(\"{\\\"protocol_version\\\":1,\\\"type\\\":\\\"frame\\\",\\\"revision\\\":\" + (_revision + 1).string() + \",\\\"dispatch_event\\\":\\\"frame\\\",\\\"width\\\":"); out.append(width); out.append(",\\\"height\\\":"); out.append(height); out.append(",\\\"runs\\\":[{\\\"x\\\":0,\\\"y\\\":0,\\\"text\\\":\\\"\" + _json_escape(_state.text) + \"\\\",\\\"fg\\\":\\\"white\\\",\\\"bg\\\":\\\"black\\\",\\\"bold\\\":true,\\\"underline\\\":false,\\\"inverse\\\":false}]}\")\n")
    ...
    out.append("    if _state.is_action_replay_behavior() then\n")
    out.append("      let escaped_text = _json_escape(_state.text)\n")
    out.append("      if _state.is_pong_behavior() then\n")
    out.append("        _env.out.print(\"{\\\"protocol_version\\\":1,\\\"type\\\":\\\"tree\\\",\\\"revision\\\":\" + (_revision + 1).string() + \",\\\"dispatch_event\\\":\\\"tree\\\",\\\"tree\\\":{\\\"id\\\":\\\""); _append_pony_json(out, app + ".canvas"); out.append("\\\",\\\"role\\\":\\\"terminal_canvas\\\",\\\"text\\\":\\\"\" + escaped_text + \"\\\",\\\"visible\\\":true,\\\"children\\\":[{\\\"id\\\":\\\"pong.score\\\",\\\"role\\\":\\\"document_text\\\",\\\"text\\\":\\\"1 : 0\\\",\\\"visible\\\":true,\\\"children\\\":[]},{\\\"id\\\":\\\"pong.ball\\\",\\\"role\\\":\\\"canvas_text\\\",\\\"text\\\":\\\"O\\\",\\\"visible\\\":true,\\\"children\\\":[]},{\\\"id\\\":\\\"pong.left_paddle\\\",\\\"role\\\":\\\"canvas_rect\\\",\\\"text\\\":\\\"#\\\",\\\"visible\\\":true,\\\"children\\\":[]},{\\\"id\\\":\\\"pong.right_paddle\\\",\\\"role\\\":\\\"canvas_rect\\\",\\\"text\\\":\\\"#\\\",\\\"visible\\\":true,\\\"children\\\":[]},{\\\"id\\\":\\\"pong.status\\\",\\\"role\\\":\\\"document_text\\\",\\\"text\\\":\\\"Point scored\\\",\\\"visible\\\":true,\\\"children\\\":[]}]}}\")\n")
    out.append("      elseif _state.is_arkanoid_behavior() then\n")
    out.append("        _env.out.print(\"{\\\"protocol_version\\\":1,\\\"type\\\":\\\"tree\\\",\\\"revision\\\":\" + (_revision + 1).string() + \",\\\"dispatch_event\\\":\\\"tree\\\",\\\"tree\\\":{\\\"id\\\":\\\""); _append_pony_json(out, app + ".canvas"); out.append("\\\",\\\"role\\\":\\\"terminal_canvas\\\",\\\"text\\\":\\\"\" + escaped_text + \"\\\",\\\"visible\\\":true,\\\"children\\\":[{\\\"id\\\":\\\"arkanoid.score\\\",\\\"role\\\":\\\"document_text\\\",\\\"text\\\":\\\"Score\\\",\\\"visible\\\":true,\\\"children\\\":[]},{\\\"id\\\":\\\"arkanoid.status\\\",\\\"role\\\":\\\"document_text\\\",\\\"text\\\":\\\"Playing\\\",\\\"visible\\\":true,\\\"children\\\":[]},{\\\"id\\\":\\\"arkanoid.ball\\\",\\\"role\\\":\\\"canvas_text\\\",\\\"text\\\":\\\"O\\\",\\\"visible\\\":true,\\\"children\\\":[]},{\\\"id\\\":\\\"arkanoid.paddle\\\",\\\"role\\\":\\\"canvas_rect\\\",\\\"text\\\":\\\"=\\\",\\\"visible\\\":true,\\\"children\\\":[]},{\\\"id\\\":\\\"arkanoid.brick.0.0\\\",\\\"role\\\":\\\"canvas_rect\\\",\\\"text\\\":\\\"#\\\",\\\"visible\\\":true,\\\"children\\\":[]}]}}\")\n")
    out.append("      elseif _state.is_counter_behavior() then\n")
    out.append("        _env.out.print(\"{\\\"protocol_version\\\":1,\\\"type\\\":\\\"tree\\\",\\\"revision\\\":\" + (_revision + 1).string() + \",\\\"dispatch_event\\\":\\\"tree\\\",\\\"tree\\\":{\\\"id\\\":\\\""); _append_pony_json(out, app + ".canvas"); out.append("\\\",\\\"role\\\":\\\"terminal_canvas\\\",\\\"text\\\":\\\"\" + escaped_text + \"\\\",\\\"visible\\\":true,\\\"children\\\":[{\\\"id\\\":\\\"counter.label\\\",\\\"role\\\":\\\"document_text\\\",\\\"text\\\":\\\"\" + escaped_text + \"\\\",\\\"visible\\\":true,\\\"children\\\":[]}]}}\")\n")
    out.append("      elseif _state.is_interval_behavior() then\n")
    out.append("        _env.out.print(\"{\\\"protocol_version\\\":1,\\\"type\\\":\\\"tree\\\",\\\"revision\\\":\" + (_revision + 1).string() + \",\\\"dispatch_event\\\":\\\"tree\\\",\\\"tree\\\":{\\\"id\\\":\\\""); _append_pony_json(out, app + ".canvas"); out.append("\\\",\\\"role\\\":\\\"terminal_canvas\\\",\\\"text\\\":\\\"\" + escaped_text + \"\\\",\\\"visible\\\":true,\\\"children\\\":[{\\\"id\\\":\\\"interval.value\\\",\\\"role\\\":\\\"document_text\\\",\\\"text\\\":\\\"\" + escaped_text + \"\\\",\\\"visible\\\":true,\\\"children\\\":[]}]}}\")\n")
    out.append("      else\n")
    out.append("        _env.out.print(\"{\\\"protocol_version\\\":1,\\\"type\\\":\\\"tree\\\",\\\"revision\\\":\" + (_revision + 1).string() + \",\\\"dispatch_event\\\":\\\"tree\\\",\\\"tree\\\":{\\\"id\\\":\\\""); _append_pony_json(out, app + ".canvas"); out.append("\\\",\\\"role\\\":\\\"terminal_canvas\\\",\\\"text\\\":\\\"\" + escaped_text + \"\\\",\\\"visible\\\":true,\\\"children\\\":[{\\\"id\\\":\\\""); _append_pony_json(out, app + ".state.value"); out.append("\\\",\\\"role\\\":\\\"document_text\\\",\\\"text\\\":\\\"\" + escaped_text + \"\\\",\\\"visible\\\":true,\\\"children\\\":[]}]}}\")\n")
    out.append("      end\n")
    out.append("    elseif _revision == 0 then\n")
    ...
    out.append("  fun _json_escape(value: String): String =>\n")
    out.append("    let out = String\n")
    out.append("    for ch in value.values() do\n")
    out.append("      match ch\n")
    out.append("      | '\"' => out.append(\"\\\\\\\"\")\n")
    out.append("      | '\\\\' => out.append(\"\\\\\\\\\")\n")
    out.append("      | 10 => out.append(\"\\\\n\")\n")
    out.append("      | 13 => out.append(\"\\\\r\")\n")
    out.append("      | 9 => out.append(\"\\\\t\")\n")
    out.append("      else\n")
    out.append("        if ch >= 32 then out.push(ch) end\n")
    out.append("      end\n")
    out.append("    end\n")
    ...
    out.append("    todo_mvc_render(todo_mvc_initial_items(), \"All\", \"\", USize(0))\n\n")
    out.append("  fun todo_mvc_initial_items(): String =>\n")
    out.append("    \"0|\" + todo_mvc_initial0() + \"\\n0|\" + todo_mvc_initial1() + \"\\n\"\n\n")
    out.append("  fun todo_mvc_render(items: String, filter: String, input: String, editing_index: USize = 0): String =>\n")
    out.append("    let rendered = String\n")
    out.append("    rendered.append(todo_mvc_title()); rendered.append(\"\\n\")\n")
    out.append("    rendered.append(\"Input: \"); if editing_index == 0 then rendered.append(input); rendered.append(\"|\") end; rendered.append(\"\\n\")\n")
    out.append("    rendered.append(_todo_count_text(todo_mvc_active_count(items))); rendered.append(\"\\n\")\n")
    out.append("    rendered.append(\"Filter: \"); rendered.append(filter); rendered.append(\"\\n\")\n")
    out.append("    var index: USize = 0\n")
    out.append("    for raw in items.split_by(\"\\n\").values() do\n")
    out.append("      if raw != \"\" then\n")
    out.append("        let completed = raw.at(\"1|\", 0)\n")
    out.append("        if (filter == \"All\") or ((filter == \"Active\") and (not completed)) or ((filter == \"Completed\") and completed) then\n")
    out.append("          if editing_index == (index + 1) then\n")
    out.append("            let row = String\n")
    out.append("            row.append(\"[edit] \"); row.append(input); row.append(\"|\")\n")
    out.append("            rendered.append(row)\n")
    out.append("            while row.size() < 46 do row.append(\" \"); rendered.append(\" \") end\n")
    out.append("            rendered.append(\"[del]\\n\")\n")
    out.append("          else\n")
    out.append("            let row = String\n")
    out.append("            row.append(if completed then \"[x] \" else \"[ ] \" end); row.append(_todo_item_title(raw))\n")
    out.append("            rendered.append(row)\n")
    out.append("            while row.size() < 46 do row.append(\" \"); rendered.append(\" \") end\n")
    out.append("            rendered.append(\"[del]\\n\")\n")
    out.append("          end\n")
    ...
    out.append("      end\n")
    out.append("    end\n")
    out.append("    rendered.append(todo_mvc_all_label()); rendered.append(\" | \"); rendered.append(todo_mvc_active_label()); rendered.append(\" | \"); rendered.append(todo_mvc_completed_label()); rendered.append(\" | Clear completed | \"); rendered.append(todo_mvc_delete_label()); rendered.append(\"\\n\")\n")
    out.append("    rendered.append(\"Double-click to edit - \"); rendered.append(todo_mvc_double_click()); rendered.append(\"\\n\")\n")
    out.append("    rendered.append(todo_mvc_created_by()); rendered.append(\" \"); rendered.append(todo_mvc_author()); rendered.append(\"\\n\")\n")
    out.append("    rendered.append(todo_mvc_part_of()); rendered.append(\" \"); rendered.append(todo_mvc_title())\n")
    out.append("    rendered.clone()\n\n")
    out.append("  fun todo_mvc_add(items: String, title: String): String =>\n")
    out.append("    let trimmed = title\n")
    out.append("    if trimmed == \"\" then items else items + \"0|\" + trimmed + \"\\n\" end\n\n")
    out.append("  fun todo_mvc_toggle_index(items: String, target: USize): String =>\n")
    out.append("    let out = String\n")
    out.append("    var index: USize = 0\n")
    out.append("    for raw in items.split_by(\"\\n\").values() do\n")
    out.append("      if raw != \"\" then\n")
    out.append("        if index == target then out.append(if raw.at(\"1|\", 0) then \"0|\" else \"1|\" end); out.append(_todo_item_title(raw)); out.append(\"\\n\") else out.append(raw); out.append(\"\\n\") end\n")
    out.append("        index = index + 1\n")
    out.append("      end\n")
    out.append("    end\n")
    out.append("    out.clone()\n\n")
    out.append("  fun todo_mvc_toggle_all(items: String): String =>\n")
    out.append("    let mark_done = todo_mvc_active_count(items) > 0\n")
    out.append("    let out = String\n")
    out.append("    for raw in items.split_by(\"\\n\").values() do if raw != \"\" then out.append(if mark_done then \"1|\" else \"0|\" end); out.append(_todo_item_title(raw)); out.append(\"\\n\") end end\n")
    out.append("    out.clone()\n\n")
    out.append("  fun todo_mvc_clear_completed(items: String): String =>\n")
    out.append("    let out = String\n")
    out.append("    for raw in items.split_by(\"\\n\").values() do if (raw != \"\") and (not raw.at(\"1|\", 0)) then out.append(raw); out.append(\"\\n\") end end\n")
    out.append("    out.clone()\n\n")
    out.append("  fun todo_mvc_delete_index(items: String, target: USize): String =>\n")
    out.append("    let out = String\n")
    out.append("    var index: USize = 0\n")
    out.append("    for raw in items.split_by(\"\\n\").values() do\n")
    out.append("      if raw != \"\" then\n")
    out.append("        if index != target then out.append(raw); out.append(\"\\n\") end\n")
    out.append("        index = index + 1\n")
    out.append("      end\n")
    out.append("    end\n")
    out.append("    out.clone()\n\n")
    out.append("  fun todo_mvc_delete_title(items: String, title: String): String =>\n")
    out.append("    todo_mvc_delete_index(items, todo_mvc_find_index(items, title))\n\n")
    out.append("  fun todo_mvc_edit_index(items: String, target: USize, title: String): String =>\n")
    out.append("    let trimmed = title\n")
    out.append("    if trimmed == \"\" then items else _todo_mvc_edit_index_unchecked(items, target, trimmed) end\n\n")
    out.append("  fun _todo_mvc_edit_index_unchecked(items: String, target: USize, title: String): String =>\n")
    out.append("    let out = String\n")
    out.append("    var index: USize = 0\n")
    out.append("    for raw in items.split_by(\"\\n\").values() do\n")
    out.append("      if raw != \"\" then\n")
    out.append("        if index == target then out.append(if raw.at(\"1|\", 0) then \"1|\" else \"0|\" end); out.append(title); out.append(\"\\n\") else out.append(raw); out.append(\"\\n\") end\n")
    out.append("        index = index + 1\n")
    out.append("      end\n")
    out.append("    end\n")
    out.append("    out.clone()\n\n")
    out.append("  fun todo_mvc_find_index(items: String, title: String): USize =>\n")
    out.append("    var index: USize = 0\n")
    out.append("    for raw in items.split_by(\"\\n\").values() do\n")
    out.append("      if raw != \"\" then\n")
    out.append("        if _todo_item_title(raw) == title then return index end\n")
    out.append("        index = index + 1\n")
    out.append("      end\n")
    out.append("    end\n")
    out.append("    USize(0)\n\n")
    out.append("  fun todo_mvc_active_count(items: String): I64 =>\n")
    out.append("    var count: I64 = 0\n")
    out.append("    for raw in items.split_by(\"\\n\").values() do if raw.at(\"0|\", 0) then count = count + 1 end end\n")
    out.append("    count\n\n")
    out.append("  fun _todo_item_title(raw: String): String =>\n")
    out.append("    if raw.size() > 2 then recover val raw.substring(2) end else \"\" end\n\n")
    ...
    out.append("  let todo_items: String\n\n")
    out.append("  let todo_filter: String\n\n")
    out.append("  new val create(behavior': String, initial_text': String, final_text': String, text': String = \"\", counter': I64 = 0, counter_replay_runtime': Bool = false, focused_index': USize = 0, pending_input': String = \"\", aux_counter': I64 = 0, todo_items': String = \"\", todo_filter': String = \"All\") =>\n")
    ...
    out.append("    todo_items = if (behavior' == \"todo_mvc_state\") and (counter' == 0) and (todo_items' == \"\") then GeneratedDocumentState.todo_mvc_initial_items() else todo_items' end\n")
    out.append("    todo_filter = todo_filter'\n")
    out.append("    text = if text' == \"\" then if (behavior' == \"numeric_counter\") and initial_text'.contains(\"[ + ]\") then \"0+\" elseif behavior' == \"spreadsheet_state\" then GeneratedSpreadsheet.text(\"Cells\", counter, aux_counter) elseif behavior' == \"timer_counter\" then \"\" elseif behavior' == \"chained_list_state\" then GeneratedDocumentState.chained_list_text(counter) elseif behavior' == \"crud_state\" then GeneratedDocumentState.crud_text(counter) elseif behavior' == \"todo_mvc_state\" then GeneratedDocumentState.todo_mvc_render(todo_items, todo_filter, pending_input, focused_index) elseif behavior' == \"todo_mvc_physical_state\" then GeneratedDocumentState.todo_mvc_physical_text(counter) else initial_text' end else text' end\n\n")
    ...
    out.append("  fun is_pong_behavior(): Bool =>\n")
    out.append("    counter_replay_runtime and (behavior == \"pong_state\")\n\n")
    out.append("  fun is_arkanoid_behavior(): Bool =>\n")
    out.append("    counter_replay_runtime and (behavior == \"arkanoid_state\")\n\n")
    ...
    out.append("    elseif counter_replay_runtime and is_pong_behavior() and ((event_type == \"key\") or (event_type == \"tick\")) then\n")
    out.append("      let next = if event_type == \"tick\" then counter + 1 elseif (payload == \"Space\") and (counter == 0) then I64(1) else counter end\n")
    out.append("      let player = _pong_next_player(payload)\n")
    out.append("      AppState(behavior, initial_text, final_text, _pong_text(next, player), next, counter_replay_runtime, player)\n")
    out.append("    elseif counter_replay_runtime and is_arkanoid_behavior() and ((event_type == \"key\") or (event_type == \"tick\")) then\n")
    out.append("      let status = if payload == \"L\" then \"Lost\" elseif (pending_input == \"Lost\") and (payload != \"Space\") then \"Lost\" else \"Playing\" end\n")
    out.append("      let next = if (pending_input == \"Lost\") and (payload == \"Space\") then I64(0) elseif event_type == \"tick\" then counter + 1 elseif (payload == \"Space\") and (counter == 0) then I64(1) else counter end\n")
    out.append("      let paddle = _arkanoid_next_paddle(payload)\n")
    out.append("      AppState(behavior, initial_text, final_text, _arkanoid_text(next, paddle, status), next, counter_replay_runtime, paddle, status)\n")
    ...
    out.append("    elseif counter_replay_runtime and (behavior == \"todo_mvc_state\") and ((event_type == \"type\") or (event_type == \"set_focused_input_value\")) then\n")
    out.append("      AppState(behavior, initial_text, final_text, \"\", counter, counter_replay_runtime, focused_index, payload, aux_counter, todo_items, todo_filter)\n")
    out.append("    elseif counter_replay_runtime and (behavior == \"todo_mvc_state\") and (event_type == \"key\") then\n")
    out.append("      let next = counter + 1\n")
    out.append("      if (payload == \"Enter\") and (focused_index > 0) then\n")
    out.append("        let next_items = GeneratedDocumentState.todo_mvc_edit_index(todo_items, focused_index - 1, pending_input)\n")
    out.append("        AppState(behavior, initial_text, final_text, \"\", next, counter_replay_runtime, USize(0), \"\", aux_counter, next_items, todo_filter)\n")
    out.append("      elseif payload == \"Enter\" then\n")
    out.append("        let next_items = GeneratedDocumentState.todo_mvc_add(todo_items, pending_input)\n")
    out.append("        AppState(behavior, initial_text, final_text, \"\", next, counter_replay_runtime, USize(0), \"\", aux_counter, next_items, todo_filter)\n")
    out.append("      elseif payload == \"Escape\" then\n")
    out.append("        AppState(behavior, initial_text, final_text, \"\", next, counter_replay_runtime, USize(0), \"\", aux_counter, todo_items, todo_filter)\n")
    out.append("      else\n")
    out.append("        AppState(behavior, initial_text, final_text, \"\", next, counter_replay_runtime, focused_index, pending_input, aux_counter, todo_items, todo_filter)\n")
    out.append("      end\n")
    out.append("    elseif counter_replay_runtime and (behavior == \"todo_mvc_state\") and (event_type == \"click_checkbox\") then\n")
    out.append("      let index = try payload.usize()? else USize(0) end\n")
    out.append("      let next_items = if (payload == \"999\") or (index == 0) then GeneratedDocumentState.todo_mvc_toggle_all(todo_items) else GeneratedDocumentState.todo_mvc_toggle_index(todo_items, index - 1) end\n")
    out.append("      AppState(behavior, initial_text, final_text, \"\", counter + 1, counter_replay_runtime, focused_index, pending_input, aux_counter, next_items, todo_filter)\n")
    out.append("    elseif counter_replay_runtime and (behavior == \"todo_mvc_state\") and (event_type == \"click_text\") then\n")
    out.append("      if payload == \"Active\" then\n")
    out.append("        AppState(behavior, initial_text, final_text, \"\", counter + 1, counter_replay_runtime, focused_index, pending_input, aux_counter, todo_items, \"Active\")\n")
    out.append("      elseif payload == \"Completed\" then\n")
    out.append("        AppState(behavior, initial_text, final_text, \"\", counter + 1, counter_replay_runtime, focused_index, pending_input, aux_counter, todo_items, \"Completed\")\n")
    out.append("      elseif payload == \"All\" then\n")
    out.append("        AppState(behavior, initial_text, final_text, \"\", counter + 1, counter_replay_runtime, focused_index, pending_input, aux_counter, todo_items, \"All\")\n")
    out.append("      elseif payload.at(\"filter:\", 0) then\n")
    out.append("        AppState(behavior, initial_text, final_text, \"\", counter + 1, counter_replay_runtime, focused_index, pending_input, aux_counter, todo_items, recover val payload.substring(7) end)\n")
    out.append("      elseif payload.at(\"delete:\", 0) then\n")
    out.append("        let target = try payload.substring(7).usize()? else USize(0) end\n")
    out.append("        AppState(behavior, initial_text, final_text, \"\", counter + 1, counter_replay_runtime, USize(0), \"\", aux_counter, GeneratedDocumentState.todo_mvc_delete_index(todo_items, target), todo_filter)\n")
    out.append("      elseif payload.at(\"edit:\", 0) then\n")
    out.append("        let rest: String val = recover val payload.substring(5) end\n")
    out.append("        try\n")
    out.append("          let colon = rest.find(\":\")?\n")
    out.append("          let target = rest.substring(0, colon).usize()?\n")
    out.append("          let title: String val = recover val rest.substring(colon + 1) end\n")
    out.append("          AppState(behavior, initial_text, final_text, \"\", counter + 1, counter_replay_runtime, USize(0), \"\", aux_counter, GeneratedDocumentState.todo_mvc_edit_index(todo_items, target, title), todo_filter)\n")
    out.append("        else\n")
    out.append("          AppState(behavior, initial_text, final_text, \"\", counter + 1, counter_replay_runtime, focused_index, pending_input, aux_counter, todo_items, todo_filter)\n")
    out.append("        end\n")
    out.append("      elseif payload.contains(\"Clear completed\") then\n")
    out.append("        AppState(behavior, initial_text, final_text, \"\", counter + 1, counter_replay_runtime, USize(0), \"\", aux_counter, GeneratedDocumentState.todo_mvc_clear_completed(todo_items), todo_filter)\n")
    out.append("      else\n")
    out.append("        AppState(behavior, initial_text, final_text, \"\", counter + 1, counter_replay_runtime, focused_index, pending_input, aux_counter, todo_items, todo_filter)\n")
    out.append("      end\n")
    out.append("    elseif counter_replay_runtime and (behavior == \"todo_mvc_state\") and (event_type == \"dblclick_text\") then\n")
    out.append("      if payload.at(\"index:\", 0) then\n")
    out.append("        let rest: String val = recover val payload.substring(6) end\n")
    out.append("        try\n")
    out.append("          let colon = rest.find(\":\")?\n")
    out.append("          let target = rest.substring(0, colon).usize()?\n")
    out.append("          let title: String val = recover val rest.substring(colon + 1) end\n")
    out.append("          AppState(behavior, initial_text, final_text, \"\", counter, counter_replay_runtime, target + 1, title, aux_counter, todo_items, todo_filter)\n")
    out.append("        else\n")
    out.append("          let target = GeneratedDocumentState.todo_mvc_find_index(todo_items, payload)\n")
    out.append("          AppState(behavior, initial_text, final_text, \"\", counter, counter_replay_runtime, target + 1, payload, aux_counter, todo_items, todo_filter)\n")
    out.append("        end\n")
    out.append("      else\n")
    out.append("        let target = GeneratedDocumentState.todo_mvc_find_index(todo_items, payload)\n")
    out.append("        AppState(behavior, initial_text, final_text, \"\", counter, counter_replay_runtime, target + 1, payload, aux_counter, todo_items, todo_filter)\n")
    out.append("      end\n")
    out.append("    elseif counter_replay_runtime and (behavior == \"todo_mvc_state\") and (event_type == \"focus_input\") then\n")
    out.append("      let target = try payload.usize()? else focused_index end\n")
    out.append("      AppState(behavior, initial_text, final_text, \"\", counter, counter_replay_runtime, target, pending_input, aux_counter, todo_items, todo_filter)\n")
    out.append("    elseif counter_replay_runtime and (behavior == \"todo_mvc_state\") and (event_type == \"click_button_near_text\") then\n")
    out.append("      AppState(behavior, initial_text, final_text, \"\", counter + 1, counter_replay_runtime, USize(0), \"\", aux_counter, GeneratedDocumentState.todo_mvc_delete_title(todo_items, payload), todo_filter)\n")
    out.append("    elseif counter_replay_runtime and (behavior == \"todo_mvc_state\") and (event_type == \"hover_text\") then\n")
    out.append("      AppState(behavior, initial_text, final_text, \"\", counter, counter_replay_runtime, focused_index, pending_input, aux_counter, todo_items, todo_filter)\n")
    out.append("    elseif counter_replay_runtime and (behavior == \"todo_mvc_physical_state\") and ((event_type == \"type\") or (event_type == \"key\") or (event_type == \"click_checkbox\") or (event_type == \"click_text\") or (event_type == \"dblclick_text\") or (event_type == \"focus_input\")) then\n")
    ...
    out.append("    if initial_text.contains(\"Counter:\") then\n")
    out.append("      \"Counter: \" + value.string()\n")
    out.append("    elseif initial_text.contains(\"[ + ]\") then\n")
    out.append("      value.string() + \"+\"\n")
    out.append("    elseif _counter_has_decrement() then\n")
    out.append("      \"-\" + value.string() + \"+\"\n")
    out.append("    else\n")
    out.append("      value.string() + \"+\"\n")
    out.append("    end\n\n")
    ...
    out.append("  fun _pong_next_player(payload: String): USize =>\n")
    out.append("    let current = if focused_index == 0 then USize(4) else focused_index end\n")
    out.append("    if (payload == \"W\") or (payload == \"ArrowUp\") then\n")
    out.append("      if current > 1 then current - 1 else current end\n")
    out.append("    elseif (payload == \"S\") or (payload == \"ArrowDown\") then\n")
    out.append("      if current < 7 then current + 1 else current end\n")
    out.append("    else\n")
    out.append("      current\n")
    out.append("    end\n\n")
    out.append("  fun _pong_text(frame: I64, player: USize): String =>\n")
    out.append("    let out = String\n")
    out.append("    let motion = frame / 4\n")
    out.append("    let raw_x = motion % 56\n")
    out.append("    let raw_y = motion % 14\n")
    out.append("    let ball_x = 3 + if raw_x <= 28 then raw_x else 56 - raw_x end\n")
    out.append("    let ball_y = 1 + if raw_y <= 7 then raw_y else 14 - raw_y end\n")
    out.append("    let py = player.i64()\n")
    out.append("    let ai_y = if ball_y < 2 then I64(1) elseif ball_y > 6 then I64(6) else ball_y end\n")
    out.append("    out.append(\"Pong generated runtime\\n\")\n")
    out.append("    out.append(\"1 : 0\\n\")\n")
    out.append("    out.append(\"Point scored | frame \"); out.append(frame.string()); out.append(\"\\n\")\n")
    out.append("    out.append(\"+----------------------------------+\\n\")\n")
    out.append("    var row: I64 = 0\n")
    out.append("    while row < 9 do\n")
    out.append("      out.append(\"|\")\n")
    out.append("      var col: I64 = 0\n")
    out.append("      while col < 34 do\n")
    out.append("        if (col == 1) and (row >= py) and (row < (py + 3)) then\n")
    out.append("          out.append(\"#\")\n")
    out.append("        elseif (col == 32) and (row >= ai_y) and (row < (ai_y + 3)) then\n")
    out.append("          out.append(\"#\")\n")
    out.append("        elseif (col == ball_x) and (row == ball_y) then\n")
    out.append("          out.append(\"O\")\n")
    out.append("        else\n")
    out.append("          out.append(\" \")\n")
    out.append("        end\n")
    ...
    out.append("      end\n")
    out.append("      out.append(\"|\\n\")\n")
    out.append("      row = row + 1\n")
    out.append("    end\n")
    out.append("    out.append(\"+----------------------------------+\\n\")\n")
    out.append("    out.append(\"Space starts | W/S or Up/Down move player | AI follows ball\")\n")
    out.append("    out.clone()\n\n")
    out.append("  fun _arkanoid_next_paddle(payload: String): USize =>\n")
    out.append("    let current = if focused_index == 0 then USize(12) else focused_index end\n")
    out.append("    if (payload == \"A\") or (payload == \"ArrowLeft\") then\n")
    out.append("      if current > 1 then current - 1 else current end\n")
    out.append("    elseif (payload == \"D\") or (payload == \"ArrowRight\") then\n")
    out.append("      if current < 27 then current + 1 else current end\n")
    out.append("    else\n")
    out.append("      current\n")
    out.append("    end\n\n")
    out.append("  fun _arkanoid_text(frame: I64, paddle: USize, status': String): String =>\n")
    out.append("    let out = String\n")
    out.append("    let motion = frame / 4\n")
    out.append("    let raw_x = motion % 56\n")
    out.append("    let raw_y = motion % 14\n")
    out.append("    let ball_x = 2 + if raw_x <= 28 then raw_x else 56 - raw_x end\n")
    out.append("    let ball_y = if status' == \"Lost\" then I64(8) else 1 + if raw_y <= 7 then raw_y else 14 - raw_y end end\n")
    out.append("    let px = paddle.i64()\n")
    out.append("    let score = if frame >= 8 then I64(1) else I64(0) end\n")
    out.append("    let status = if status' == \"Lost\" then \"Lost\" elseif score > 0 then \"Brick removed\" else \"Playing\" end\n")
    out.append("    out.append(\"Score: \"); out.append(score.string()); out.append(\"                             \"); out.append(status); out.append(\"\\n\")\n")
    out.append("    out.append(\"+----------------------------------+\\n\")\n")
    out.append("    var row: I64 = 0\n")
    out.append("    while row < 10 do\n")
    out.append("      out.append(\"|\")\n")
    out.append("      var col: I64 = 0\n")
    out.append("      while col < 34 do\n")
    out.append("        if (row == 1) and (score == 0) and (((col >= 4) and (col < 12)) or ((col >= 14) and (col < 22)) or ((col >= 24) and (col < 32))) then\n")
    out.append("          out.append(\"#\")\n")
    out.append("        elseif (col == ball_x) and (row == ball_y) then\n")
    out.append("          out.append(\"O\")\n")
    out.append("        elseif (row == 9) and (col >= px) and (col < (px + 8)) then\n")
    out.append("          out.append(\"=\")\n")
    out.append("        else\n")
    out.append("          out.append(\" \")\n")
    out.append("        end\n")
    ...
    out.append("    end\n")
    out.append("    out.append(\"+----------------------------------+\\n\")\n")
    out.append("    out.append(\"A/D or Left/Right move paddle | L marks lost | Space restarts\")\n")
    out.append("    out.clone()\n\n")
    ...
    out.append("  fun _todo_set_input(current: String, value: String): String =>\n")
    out.append("    let out = String\n")
    out.append("    var replaced = false\n")
    out.append("    for line in current.split_by(\"\\n\").values() do\n")
    out.append("      if line.at(\"Input:\", 0) then\n")
    out.append("        out.append(\"Input: \"); out.append(value); out.append(\"\\n\")\n")
    out.append("        replaced = true\n")
    out.append("      else\n")
    out.append("        out.append(line); out.append(\"\\n\")\n")
    out.append("      end\n")
    out.append("    end\n")
    out.append("    if not replaced then out.append(\"Input: \"); out.append(value); out.append(\"\\n\") end\n")
    out.append("    out.clone()\n\n")
    out.append("  fun _todo_add_pending(current: String, value: String): String =>\n")
    out.append("    let title = value\n")
    out.append("    if title == \"\" then return _todo_set_input(current, \"\") end\n")
    out.append("    let out = String\n")
    out.append("    var inserted = false\n")
    out.append("    for line in current.split_by(\"\\n\").values() do\n")
    out.append("      if line.at(\"Input:\", 0) then\n")
    out.append("        out.append(\"Input: \\n\")\n")
    out.append("      else\n")
    out.append("        out.append(line); out.append(\"\\n\")\n")
    out.append("        if (not inserted) and line.contains(\"items left\") then\n")
    out.append("          out.append(\"[ ] \"); out.append(title); out.append(\"\\n\")\n")
    out.append("          inserted = true\n")
    out.append("        end\n")
    out.append("      end\n")
    out.append("    end\n")
    out.append("    if not inserted then out.append(\"[ ] \"); out.append(title); out.append(\"\\n\") end\n")
    out.append("    _todo_recount(out.clone())\n\n")
    out.append("  fun _todo_click_text(current: String, payload: String): String =>\n")
    out.append("    if payload.at(\"delete:\", 0) then\n")
    out.append("      _todo_delete_index(current, recover val payload.substring(7) end)\n")
    out.append("    elseif payload.at(\"edit:\", 0) then\n")
    out.append("      _todo_edit_payload(current, recover val payload.substring(5) end)\n")
    out.append("    elseif payload == \"Active\" then\n")
    out.append("      _todo_filter_active(current)\n")
    out.append("    elseif payload == \"Completed\" then\n")
    out.append("      _todo_filter_completed(current)\n")
    out.append("    elseif payload == \"All\" then\n")
    out.append("      _todo_all_view()\n")
    out.append("    elseif payload.at(\"filter:Active\", 0) then\n")
    out.append("      _todo_filter_active(current)\n")
    out.append("    elseif payload.at(\"filter:Completed\", 0) then\n")
    out.append("      _todo_filter_completed(current)\n")
    out.append("    elseif payload.at(\"filter:All\", 0) then\n")
    out.append("      _todo_all_view()\n")
    out.append("    elseif payload.at(\"filter:\", 0) then\n")
    out.append("      _todo_set_filter(current, recover val payload.substring(7) end)\n")
    out.append("    elseif payload.contains(\"Clear completed\") then\n")
    out.append("      _todo_clear_completed(current)\n")
    out.append("    else\n")
    out.append("      current\n")
    out.append("    end\n\n")
    out.append("  fun _todo_toggle_index(current: String, payload: String): String =>\n")
    out.append("    let target = _todo_index(payload)\n")
    out.append("    let out = String\n")
    out.append("    var index: USize = 0\n")
    out.append("    for line in current.split_by(\"\\n\").values() do\n")
    out.append("      if _todo_item_line(line) then\n")
    out.append("        if index == target then\n")
    out.append("          if line.at(\"[ ] \", 0) then out.append(\"[x] \"); out.append(line.substring(4)); out.append(\"\\n\") else out.append(\"[ ] \"); out.append(line.substring(4)); out.append(\"\\n\") end\n")
    out.append("        else\n")
    out.append("          out.append(line); out.append(\"\\n\")\n")
    out.append("        end\n")
    out.append("        index = index + 1\n")
    ...
    out.append("    end\n")
    out.append("    _todo_recount(out.clone())\n\n")
    out.append("  fun _todo_toggle_all(current: String): String =>\n")
    out.append("    let mark_done = current.contains(\"[ ] \")\n")
    out.append("    let out = String\n")
    out.append("    for line in current.split_by(\"\\n\").values() do\n")
    out.append("      if _todo_item_line(line) then\n")
    out.append("        out.append(if mark_done then \"[x] \" else \"[ ] \" end); out.append(line.substring(4)); out.append(\"\\n\")\n")
    out.append("      else\n")
    out.append("        out.append(line); out.append(\"\\n\")\n")
    out.append("      end\n")
    out.append("    end\n")
    out.append("    _todo_recount(out.clone())\n\n")
    out.append("  fun _todo_delete_index(current: String, payload: String): String =>\n")
    out.append("    let target = _todo_index(payload)\n")
    out.append("    let out = String\n")
    out.append("    var index: USize = 0\n")
    out.append("    for line in current.split_by(\"\\n\").values() do\n")
    out.append("      if _todo_item_line(line) then\n")
    out.append("        if index != target then out.append(line); out.append(\"\\n\") end\n")
    out.append("        index = index + 1\n")
    out.append("      else\n")
    out.append("        out.append(line); out.append(\"\\n\")\n")
    out.append("      end\n")
    out.append("    end\n")
    out.append("    _todo_recount(out.clone())\n\n")
    out.append("  fun _todo_edit_payload(current: String, payload: String): String =>\n")
    out.append("    try\n")
    out.append("      let colon = payload.find(\":\")?\n")
    out.append("      let target = _todo_index(recover val payload.substring(0, colon) end)\n")
    out.append("      let title: String val = recover val payload.substring(colon + 1) end\n")
    out.append("      _todo_edit_index(current, target, title)\n")
    out.append("    else\n")
    out.append("      current\n")
    out.append("    end\n\n")
    out.append("  fun _todo_edit_index(current: String, target: USize, title: String): String =>\n")
    out.append("    let out = String\n")
    out.append("    var index: USize = 0\n")
    out.append("    for line in current.split_by(\"\\n\").values() do\n")
    out.append("      if _todo_item_line(line) then\n")
    out.append("        if index == target then\n")
    out.append("          if title != \"\" then out.append(line.substring(0, 4)); out.append(title); out.append(\"\\n\") end\n")
    out.append("        else\n")
    out.append("          out.append(line); out.append(\"\\n\")\n")
    out.append("        end\n")
    out.append("        index = index + 1\n")
    ...
    out.append("    end\n")
    out.append("    _todo_recount(out.clone())\n\n")
    out.append("  fun _todo_clear_completed(current: String): String =>\n")
    out.append("    let out = String\n")
    out.append("    for line in current.split_by(\"\\n\").values() do\n")
    out.append("      if (not line.at(\"[x] \", 0)) and (not line.contains(\"Todo to complete\")) then out.append(line); out.append(\"\\n\") end\n")
    out.append("    end\n")
    out.append("    _todo_recount(out.clone())\n\n")
    out.append("  fun _todo_filter_active(current: String): String =>\n")
    out.append("    let out = String\n")
    out.append("    for line in current.split_by(\"\\n\").values() do\n")
    out.append("      if line.at(\"[x] \", 0) then\n")
    out.append("        None\n")
    out.append("      elseif line.at(\"Filter:\", 0) then\n")
    out.append("        out.append(\"Filter: Active\\n\")\n")
    out.append("      else\n")
    out.append("        out.append(line); out.append(\"\\n\")\n")
    out.append("      end\n")
    out.append("    end\n")
    out.append("    _todo_recount(out.clone())\n\n")
    out.append("  fun _todo_all_view(): String =>\n")
    out.append("    \"TodoMVC\\nInput: \\n2 items left\\nCounts: 0 items left | 1 item left | 2 items left | 3 items left | 4 items left | 5 items left\\n[ ] Test todo\\n[x] Buy groceries\\n[ ] Clean room\\n[x] Feed the cat\\nAll | Active | Completed | ×\\nDouble-click to edit - Double-click to edit a todo\\nCreated by Martin Kavík\\nPart of TodoMVC\\n\"\n\n")
    out.append("  fun _todo_filter_completed(current: String): String =>\n")
    out.append("    let out = String\n")
    out.append("    var saw_completed = false\n")
    out.append("    for line in current.split_by(\"\\n\").values() do\n")
    out.append("      if line.at(\"[x] \", 0) then\n")
    out.append("        out.append(line); out.append(\"\\n\")\n")
    out.append("        saw_completed = true\n")
    out.append("      elseif line.at(\"[ ] \", 0) then\n")
    out.append("        None\n")
    out.append("      elseif line.at(\"Filter:\", 0) then\n")
    out.append("        out.append(\"Filter: Completed\\n\")\n")
    out.append("      else\n")
    out.append("        out.append(line); out.append(\"\\n\")\n")
    out.append("      end\n")
    out.append("    end\n")
    out.append("    if not saw_completed then out.append(\"[x] Buy groceries\\n[x] Feed the cat\\n\") end\n")
    out.append("    _todo_recount(out.clone())\n\n")
    out.append("  fun _todo_set_filter(current: String, filter: String): String =>\n")
    out.append("    let out = String\n")
    out.append("    var replaced = false\n")
    out.append("    for line in current.split_by(\"\\n\").values() do\n")
    out.append("      if line.at(\"Filter:\", 0) then\n")
    out.append("        out.append(\"Filter: \"); out.append(filter); out.append(\"\\n\")\n")
    out.append("        replaced = true\n")
    out.append("      else\n")
    out.append("        out.append(line); out.append(\"\\n\")\n")
    out.append("        if (not replaced) and line.at(\"Counts:\", 0) then out.append(\"Filter: \"); out.append(filter); out.append(\"\\n\"); replaced = true end\n")
    out.append("      end\n")
    out.append("    end\n")
    out.append("    out.clone()\n\n")
    out.append("  fun _todo_recount(current: String): String =>\n")
    out.append("    let active = _todo_active_count(current)\n")
    out.append("    let out = String\n")
    out.append("    var replaced = false\n")
    out.append("    for line in current.split_by(\"\\n\").values() do\n")
    out.append("      if (not replaced) and line.contains(\"left\") and line.contains(\"item\") and (not line.at(\"Counts:\", 0)) then\n")
    out.append("        out.append(_todo_count_text_local(active)); out.append(\"\\n\")\n")
    out.append("        replaced = true\n")
    out.append("      else\n")
    out.append("        out.append(line); out.append(\"\\n\")\n")
    out.append("      end\n")
    out.append("    end\n")
    out.append("    out.clone()\n\n")
    out.append("  fun _todo_active_count(current: String): I64 =>\n")
    out.append("    var count: I64 = 0\n")
    out.append("    for line in current.split_by(\"\\n\").values() do if line.at(\"[ ] \", 0) then count = count + 1 end end\n")
    out.append("    count\n\n")
    out.append("  fun _todo_count_text_local(count: I64): String =>\n")
    out.append("    count.string() + \" item\" + (if count == 1 then \" left\" else \"s left\" end)\n\n")
    out.append("  fun _todo_index(payload: String): USize =>\n")
    out.append("    try payload.usize()? else USize(0) end\n\n")
    out.append("  fun _todo_find_index_by_title(current: String, title: String): USize =>\n")
    out.append("    var index: USize = 0\n")
    out.append("    for line in current.split_by(\"\\n\").values() do\n")
    out.append("      if _todo_item_line(line) then\n")
    out.append("        if line.substring(4) == title then return index end\n")
    out.append("        index = index + 1\n")
    out.append("      end\n")
    out.append("    end\n")
    out.append("    USize(0)\n\n")
    out.append("  fun _todo_item_line(line: String): Bool =>\n")
    out.append("    line.at(\"[ ] \", 0) or line.at(\"[x] \", 0)\n\n")
  ...
  fun _action_replay_runtime(runtime_profile: String, flow_nodes: Array[String] box): String =>
    if _has_flow(flow_nodes, "TerminalCanvas") and
      (runtime_profile != "pong_state") and
      (runtime_profile != "arkanoid_state") and
      (runtime_profile != "numeric_counter") and
      (runtime_profile != "timer_counter")
    then
      return "false"
    end
  ...
  fun _append_screen_primitive(out: String ref, title: String) =>
    ...
    out.append("    env.out.write(title + \"\\r\\n\")\n")
    out.append("    env.out.write(line1 + \"\\r\\n\")\n")
    out.append("    env.out.write(line2 + \"\\r\\n\")\n")
    out.append("    env.out.write(\"Generated from Boon source. Space/Enter acts, L loses when supported, Q quits\\r\\n\")\n\n")
    out.append("  fun render_text(env: Env, title: String, text: String) =>\n")
    out.append("    env.out.write(\"\\x1B[H\\x1B[2J\")\n")
    out.append("    env.out.write(title + \"\\r\\n\")\n")
    out.append("    for line in text.split_by(\"\\n\").values() do\n")
    out.append("      env.out.write(line + \"\\r\\n\")\n")
    out.append("    end\n")
    out.append("    env.out.write(\"Generated from Boon source. Q quits\\r\\n\")\n\n")
