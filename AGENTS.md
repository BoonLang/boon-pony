...
- when launching any command that opens a window from an agent session, use `cosmic-background-launch --workspace boon-pony -- ...` so the window opens in the `boon-pony` background workspace and does not steal the user's focus
- when launching the native SDL GUI playground manually from an agent session, use `cosmic-background-launch --workspace boon-pony -- ...` or `tools/run_gui_background.sh` so the window opens in the `boon-pony` background workspace; live SDL launches should prefer `SDL_VIDEODRIVER=wayland` and `SDL_RENDER_DRIVER=gpu`, while scripted verification should keep using the dummy video driver
