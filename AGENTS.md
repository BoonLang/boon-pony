...
- when launching the native SDL GUI playground manually from an agent session, use `cosmic-background-launch -- ...` or `tools/run_gui_background.sh` so the window opens in the agent/background workspace and does not steal the user's focus; live SDL launches should prefer `SDL_VIDEODRIVER=wayland` and `SDL_RENDER_DRIVER=gpu`, while scripted verification should keep using the dummy video driver
